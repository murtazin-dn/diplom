package com.example.diplom.presentation.myprofile
...
class MyProfileFragment : Fragment() {
    ...
    private fun handleMyProfileInfo(personInfo: MyProfileResponse) = with(binding) {
        ...
        if (!personInfo.icon.isNullOrEmpty()) {
            val imgAddress = "${BASE_URL}users/photo/${personInfo.icon}"
            Glide
                .with(requireContext())
                .load(imgAddress)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder) {
            ...
            if (!item.user.icon.isNullOrEmpty()) {
                val imgAddress = "${BASE_URL}users/photo/${item.user.icon}"
                Glide
